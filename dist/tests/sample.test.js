...
describe("obj path", () => {
    ...
    it("should succeed on an empty object", () => {
        ...
        expect(lib_1.ObjPath.valuesAtPath(path, obj)).toEqual([]);
    });
    it("should succeed on an empty path", () => {
        const obj = { key: "value" };
        const path = [];
        expect(lib_1.ObjPath.valuesAtPath(path, obj)).toEqual([obj]);
    });
    ...
    it("should succeed on a null path", () => {
        const obj = { key: "value" };
        expect(lib_1.ObjPath.valuesAtPath(null, obj)).toEqual([]);
    });
    ...
    it("should extract a value when the path is short", () => {
        ...
        expect(lib_1.ObjPath.valuesAtPath(path, testObj)).toEqual([expected]);
    });
    it("should extract a value when the object has an array", () => {
        const path = ["enrollment", "wizards", "name"];
        const expected = ["bob", "george", "harry"];
        expect(lib_1.ObjPath.valuesAtPath(path, testWithArrayObj)).toEqual(expected);
    });
